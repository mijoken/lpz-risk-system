...
def _decode_imerg_v07_file(
    path: str | Path,
    *,
    source_id: str,
    gauge_adjusted: bool,
) -> CanonicalRainfallField:
    """Decode the common IMERG V07 half-hourly HDF5 grid.

    Product-specific provenance is supplied by the public wrapper.
    """
    p = Path(path)
    ...
    with h5py.File(p, "r") as h:
        ...
        if fill is not None:
            rain[np.isclose(
                rain,
                float(np.asarray(fill).reshape(-1)[0]),
            )] = np.nan
        ...
        if str(units).lower().replace(" ", "") not in {
            "mm/hr",
            "mmhr-1",
            "mmh-1",
        }:
            raise ValueError(
                f"unexpected IMERG precipitation units: {units!r}"
            )
...
def decode_imerg_final_v07_file(
    path: str | Path,
) -> CanonicalRainfallField:
    """Decode IMERG Final V07 with Final-product provenance."""
    return _decode_imerg_v07_file(
        path,
...
def decode_imerg_early_v07_file(
    path: str | Path,
) -> CanonicalRainfallField:
    """Decode IMERG Early V07 with Early-product provenance."""
    return _decode_imerg_v07_file(
        path,
